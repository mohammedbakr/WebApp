            ...
            <form action="{{ route('customer.address.update', [$customer->id, $address->id]) }}" method="post" class="form" enctype="multipart/form-data">
                ...
                <div class="box-body">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="alias" id="alias" placeholder="{{trans('main.address.Home or Office')}}" class="form-control" value="{{ old('alias') ?? $address->alias }}">
                    </div>
                    <div class="form-group">
                        <label for="address_1">{{trans('main.address.Address')}} 1 <span class="text-danger">*</span></label>
                        <input type="text" name="address_1" id="address_1" placeholder="{{trans('main.address.Address')}} 1" class="form-control" value="{{ old('address_1') ?? $address->address_1 }}">
                    </div>
                    <div class="form-group">
                        <label for="address_2">{{trans('main.address.Address')}} 2 </label>
                        <input type="text" name="address_2" id="address_2" placeholder="{{trans('main.address.Address')}} 2" class="form-control" value="{{ old('address_2') ?? $address->address_2 }}">
                    </div>
                    <div class="form-group">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="zip" id="zip" placeholder="{{trans('main.address.Zip Code')}}" class="form-control" value="{{ old('zip') ?? $address->zip }}">
                    </div>
                    <div class="form-group">
                        <label for="phone">{{trans('main.address.Your Phone')}} </label>
                        <input type="text" name="phone" id="phone" placeholder="{{trans('main.address.Your Phone')}}" class="form-control" value="{{ old('phone') ?? $address->phone }}">
                    </div>
                </div>
                <!-- /.box-body -->
                <div class="box-footer">
                    <div class="btn-group">
                        <a href="{{ route('accounts', ['tab' => 'address']) }}" class="btn btn-default">{{trans('main.address.Back')}}</a>
                        <button type="submit" class="btn btn-primary">{{trans('main.address.Update')}}</button>
                    </div>
                </div>
            </form>
        </div>
